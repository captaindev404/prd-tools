...
enum Endpoint {
    ...
    case getHero(id: UUID, includeStories: Bool)
    ...
    // MARK: - Stories
    case getStories(heroId: UUID?, limit: Int, offset: Int, includeIllustrations: Bool)
    case getStory(id: UUID, includeIllustrations: Bool)
    ...
    case getCustomEvent(id: UUID)
    case createCustomEvent(data: CustomEventCreateRequest)
    case updateCustomEvent(id: UUID, data: CustomEventUpdateRequest)
    case deleteCustomEvent(id: UUID)
    case enhanceCustomEvent(id: UUID)
    ...
    var path: String {
        ...
        case .getHero(let id, _):
            return "/api/heroes/\(id.uuidString)"
        case .createHero:
            return "/api/heroes"
        ...
        case .generateAvatar(let heroId, _):
            return "/api/heroes/\(heroId.uuidString)/avatar"

        // Stories
        case .getStories:
            return "/api/stories"
        case .getStory(let id, _):
            return "/api/stories/\(id.uuidString)"
        case .createStory:
            return "/api/stories"
        ...
        case .generateAudio(let storyId, _, _):
            return "/api/stories/\(storyId.uuidString)/audio"
        case .generateIllustrations(let storyId):
            return "/api/stories/\(storyId.uuidString)/illustrations"
        case .getIllustrationStatus(let storyId):
            return "/api/stories/\(storyId.uuidString)/illustrations/status"

        // Custom Events
        case .getCustomEvents:
            return "/api/custom-events"
        case .getCustomEvent(let id):
            return "/api/custom-events/\(id.uuidString)"
        ...
        case .enhanceCustomEvent(let id):
            return "/api/custom-events/\(id.uuidString)/enhance"
    ...
    var queryItems: [URLQueryItem]? {
        ...
        case .getStories(let heroId, let limit, let offset, let includeIllustrations):
            ...
            if let heroId = heroId {
                items.append(URLQueryItem(name: "heroId", value: heroId.uuidString))
            }
            return items
